    ...
    try {
      ...
      for (const [key, groupFiles] of Object.entries(groups)) {
        ...
        (result.models || []).forEach(modelResp => {
          ...
          stats[name].rows.push({ discharge: key, justification, justification_threshold: pass, count_threshold: countPass });
        });
      }
  ...
  async uploadAutomatedPredict(req, res) {
    ...
    try {
      ...
      (result.models || []).forEach(modelResp => {
        ...
        const countPass = history.length >= countThresh && history.slice(-countThresh).every(v => v === 1) ? 1 : 0;
        session.stats[name].rows.push({ discharge: dischargeId, justification, justification_threshold: pass, count_threshold: countPass });
      });
